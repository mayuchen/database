...
import com.pl.iframe.insert.CoachInsertIFrame.CloseActionListener;
import com.pl.iframe.insert.CoachInsertIFrame.ISBNkeyListener;

public class RefereeInsertIFrame extends JInternalFrame{
	...
	public RefereeInsertIFrame() {
		...
		label_3_1.setText("业务等级：");
		panel.add(label_3_1);
		  ability=   new   JTextField();
		  ability.setColumns(1);
		...
		buttonadd.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(CAID.getText().equals("")||coachName.getText().equals(""))
					{JOptionPane.showMessageDialog(null, "请输入裁判姓名，ID");
						return;
					}
				ResultSet test=Database.executeQuery("select * from referee where referee_id ="+CAID.getText());
				try {
					if(test.next())
					{
					...
					{
						String sql="INSERT INTO referee (referee_id,referee_name,"
								+ "referee_birthday";

						if(!ability.getText().equals(""))
							sql=sql+",referee_standard";
